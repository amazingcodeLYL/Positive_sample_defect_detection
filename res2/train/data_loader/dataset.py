...
def is_image_file(filename):  # 定义一个判断是否是图片的函数
    return filename.suffix in [".png", ".jpg", ".jpeg", ".bmp"]
...
input_transform_1 = transforms.Compose([
    transforms.ToTensor(),  ##如果是numpy或者pil image格式，会将[0,255]转为[0,1]，并且(hwc)转为(chw)
    # transforms.Normalize(*mean_std)  # [0,1]  ---> 符合imagenet的范围[-2.117,2.248][,][,]
])
...
class train_dataset(data.Dataset):
    def __init__(self, data_path='', size_w=256, size_h=256, flip=0, input_channle=3):
        super(train_dataset, self).__init__()
        self.list = [str(x.stem) for x in Path(data_path).joinpath('src').iterdir() if is_image_file(x)]
    ...
    def __getitem__(self, index):
        initial_path = str(Path(self.data_path).joinpath('src').joinpath(self.list[index] + '.bmp'))
        semantic_path = str(Path(self.data_path).joinpath('label').joinpath(self.list[index] + '.bmp'))

        try:
            if self.input_channle == 1:
                initial_image = Image.open(initial_path).convert('P')
            else:
        ...
        if self.input_channle == 1:
            initial_image = input_transform_1(initial_image)
        else:
