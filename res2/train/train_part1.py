...
def train():
    ...
    batch_size = 15
    data_path = "./part1_new/"
    model_save = "/tmp/data/part1"
    train_datatset_ = train_dataset(data_path, size_w, size_h, is_flip, input_channle)
    train_loader = torch.utils.data.DataLoader(dataset=train_datatset_, batch_size=batch_size, shuffle=True)

    net = CSFNet(2, input_channle)
    net.cuda()
    ...
    criterion = nn.CrossEntropyLoss(reduction='mean', ignore_index=255, weight=torch.FloatTensor([1, 3]).cuda())
