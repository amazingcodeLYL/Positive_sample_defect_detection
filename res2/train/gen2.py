...
def get_random_shape():
    ...
    if black_type == 0:
        ...
        src_tmp = rotato(temp, random.randint(0, 360), random.randint(3, 5) / 10,
                         center=(random.randint(30, 100), random.randint(30, 100)))
        return src_tmp


def gen():
    print("part2 train img generate begin:")
    ok_part2_img_path = '../raw_data/round_train/part2/OK_Images'
    ...
    while cnt < pic_num:
        ...
        # cv.waitKey(0)
        Path_lib("./part2_new/src/").mkdir(parents=True, exist_ok=True)
        Path_lib("./part2_new/label/").mkdir(parents=True, exist_ok=True)
        cv.imwrite("./part2_new/src/" + str(cnt) + ".bmp", cv.GaussianBlur(src, (7, 7), 2.5))
        cv.imwrite("./part2_new/label/" + str(cnt) + ".bmp", label_out)
        cnt += 1
        if cnt % 1000 == 0:
            print('part2 gen ', cnt / pic_num)
