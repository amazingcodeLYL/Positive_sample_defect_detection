...
def eval():
    model = CSFNet(2, input_channle=input_channle)
    data_path = '../raw_data/round_test/part1/TC_Images/'
    # model_path = './ckpt_pre3p1_bs15/res2net/model/netG_final.pth'
    model_path = '/tmp/data/part1/netG_final.pth'
    ...
    for (tmp_img, size_list) in evalset:
        ...
        for x in range(predictions.shape[0]):
            ...
            dump_json(result, str(Path(nameset[cnt]).stem), 1)
            cnt += 1
            if cnt % 1000 == 0:
