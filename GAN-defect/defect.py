...
class DefectAdder(object):
    ...
    def add_defect(self, input):
        ...
        size_ratio = np.random.uniform(self.size_range[0], self.size_range[1]) #从均匀分布中抽取样本。
...
if __name__=="__main__":
    defect=DefectAdder(mode='geometry', defect_shape=('line',), normal_only=True)
    image=Image.open('/home/lyl/object detection/GAN-defect/demo/r.bmp').convert('RGB')
    # print(image)

    output=defect.add_defect(image)
    image.show()
    # print(output)