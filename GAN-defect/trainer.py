...
class Trainer():
    def __init__(self, opt, model, optimizer, lr_schedule, train_data_loader,
        ...
        self.criterion = nn.BCELoss() #是单目标二分类交叉熵函数
    ...
    def train(self):
        ...
        for epoch in range(self.opt.max_epoch):
            progressbar = tqdm(self.train_data_loader) #可扩展的Python进度条 ，在 Python 长循环中添加一个进度提示信息
            d_loss = AverageMeter()
            g_loss = AverageMeter()
            c_loss = AverageMeter()
            s_loss = AverageMeter()
            for ii, (imgs, _) in enumerate(progressbar):
                ...
                if (ii + 1) % self.opt.d_every == 0:
                    ...
                    fake_img = self.netg(defect).detach() # 避免梯度传到G，因为G不用更新, detach分离
                    #detach() 切断反向传播 不会有梯度计算
                    fake_output = self.netd(fake_img)
                    error_d_fake = self.criterion(fake_output, fake_labels)
                    error_d_fake.backward()
                    self.optimizer_d.step()
                    d_loss.update(error_d_real + error_d_fake) #损失包括判真损失和判假损失
                ...
                if (ii + 1) % self.opt.g_every == 0:
                    ...
                    g_loss.update(error_g) #得到的假的图片与真实的图片的label的loss
                ...
                #     target = target.long()
                #     loss = ls.cross_entropy2d(seg_output, target)
                #     loss /= len(defect)
                #     loss.backward()
                ...
                #         if not os.path.exists(self.opt.save_path):
                #             os.makedirs(self.opt.save_path)
                #         tv.utils.save_image(seg_output,os.path.join(self.opt.save_path,'{}_seg.jpg'.format(ii)),normalize=True,range=(-1,1))
            ...
            if (epoch + 1) % self.opt.checkpoint_interval == 0:
                ...
                # state_s={'net':self.nets.state_dict(),'optimizer':self.optimizer_s.state_dict(),'epoch':epoch}
                print('saving checkpoints...')
                torch.save(state_d, os.path.join(self.opt.work_dir, 'd_ckpt_e{epoch + 1}.pth'))
                torch.save(state_g, os.path.join(self.opt.work_dir, 'g_ckpt_e{epoch + 1}.pth'))
                # torch.save(state_s, os.path.join(self.opt.work_dir, 's_ckpt_e{epoch + 1}.pth'))
    ...
    def validate(self):
        ...
        for ii, (imgs, _) in enumerate(progressbar):
            ...
            if self.opt.with_segmentation:
                ...
                progressbar.set_description(
                    'Acc: {metrics[0]:.5f}, Acc_cls: {metrics[1]:.5f}, MIU: {metrics[2]:.5f}, Fwavacc: {metrics[3]:.5f}')
            if self.opt.debug:
